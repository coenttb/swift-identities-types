...
public struct Login: HTML {
    ...
    public var body: some HTML {
        ...
        PageModule(theme: .login) {
            form {
                VStack {
                    Input.default(Coenttb_Identity.API.Login.CodingKeys.email)
                        .type(.email)
                        .placeholder(String.email.capitalizingFirstLetter().description)
                        .focusOnPageLoad()
                    
                    Input.default(Coenttb_Identity.API.Login.CodingKeys.password)
                        .type(.password)
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            form.addEventListener('submit', async function(event) {
                ...
                const email = formData.get('\#(Coenttb_Identity.API.Login.CodingKeys.email.rawValue)');      
                const password = formData.get('\#(Coenttb_Identity.API.Login.CodingKeys.password.rawValue)'); 
                ...
                try {
                    const response = await fetch(form.action, {
                        ...
                        body: new URLSearchParams({
                             \#(Coenttb_Identity.API.Login.CodingKeys.email.rawValue): email,      
                             \#(Coenttb_Identity.API.Login.CodingKeys.password.rawValue): password
                        }).toString()  
                    });
